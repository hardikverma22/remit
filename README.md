...
[![npm version](https://badge.fury.io/js/@example%2Fremit.svg)](https://badge.fury.io/js/@example%2Fremit)
...
[File it here](https://github.com/hardik/remit/issues).
...
export const addComment = mutation({
  ...
  handler: async (ctx, args) => {
    return await ctx.runMutation(components.remit.lib.add, {
      text: args.text,
...
import { registerRoutes } from "@agent-governance/convex";
import { components } from "./_generated/api";

const http = httpRouter();

registerRoutes(http, components.remit, {
  pathPrefix: "/comments",
});
